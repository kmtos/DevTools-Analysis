...
FILES = []
with open('/afs/cern.ch/work/k/ktos/public/DevAnalysis/CMSSW_8_1_0/src/DevTools/Analyzer/python/data/SAMPLE_NAME/summedWeights.pkl', 'rb') as f:
  pklSumWeights =  pickle.load(f)
  pklSumWeightShift = pklSumWeights[6]
  pklSumWeightNorm  = pklSumWeights[-1]
...
for FILE in FILES:
  for iev,event in enumerate(FILE):
    ...
    if ISMC:
      ...
      eventGenWeight = lheEvent.product().weights()[6].wgt #1 #genEvent.weight()
      genWeight = eventGenWeight * LUMI * XSEC * .001 * eventGenWeight / SUMMEDWEIGHTS * pklSumWeightNorm / pklSumWeightShift   
