...
class MuMuTauTauAnalysis(AnalysisBase):
    ...
    def selectCandidates(self):
        ...
        for quad in itertools.permutations(leps,4):
            # require mmmt
            if not isinstance(quad[0],Muon): continue
            if not isinstance(quad[1],Muon): continue
            if not isinstance(quad[2],Muon): continue
            if not isinstance(quad[3],Tau): continue
            # charge OS
