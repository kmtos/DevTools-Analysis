...
#      TrackCorr_vtx += [val]
  ...
  fileNameFR = ROOT.TFile("/afs/cern.ch/work/k/ktos/public/CMSSW_8_0_17/src/AnalyzerGeneratorRecoVariousFunctions/Analyzer/FILE_TESTS/TAUFAKERATES_DOWN.root")
  histFR0_ = fileNameFR.Get("FinalFakeRateDMtoMedIsoOnlyEtavsPtDM0")
  histFR1_ = fileNameFR.Get("FinalFakeRateDMtoMedIsoOnlyEtavsPtDM1")
  histFR10_ = fileNameFR.Get("FinalFakeRateDMtoMedIsoOnlyEtavsPtDM10")
...
for FILE in FILES:
  for iev,event in enumerate(FILE):
    ...
    if ISMC:
      ...
      xaxis = histFR0_.GetXaxis()
      yaxis = histFR0_.GetYaxis()
      binx = xaxis.FindBin(th.pt()  )
      biny = yaxis.FindBin(math.fabs(th.eta()) )
      if   th.decayMode() == 0:  rate = histFR0_.GetBinContent(binx, biny)
      elif th.decayMode() == 1:  rate = histFR1_.GetBinContent(binx, biny)
      elif th.decayMode() == 10: rate = histFR10_.GetBinContent(binx, biny)
      else: rate = -1000
      ...
      while rate == 0 and binx > 0:
        ...
        binx -= 1
        if th.decayMode() == 0: rate = histFR0_.GetBinContent(binx, biny)
        if th.decayMode() == 1: rate = histFR1_.GetBinContent(binx, biny)
        if th.decayMode() == 10: rate = histFR10_.GetBinContent(binx, biny)
      if rate < 0: rate = 0
