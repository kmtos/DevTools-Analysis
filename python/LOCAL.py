...
FILES = []
FILES.append( Events("RegionB_selection_5.root"))

with open('/afs/cern.ch/work/k/ktos/public/DevAnalysis/CMSSW_8_1_0/src/DevTools/Analyzer/python/data/SUSYGluGluToHToAA_AToMuMu_AToTauTau_M-11_TuneCUETP8M1_13TeV_madgraph_pythia8/summedWeights.pkl', 'rb') as f:
  pklWeights =  pickle.load(f)
  pklWeight = pklWeights[1]
 

print "NUM FILES=", len(FILES)
csvFile = open('/afs/cern.ch/work/k/ktos/public/DevAnalysis/CMSSW_8_1_0/src/DevTools/Analyzer/python/BSUB/IRNAME_EventList.csv','w')
...
for FILE in FILES:
  for iev,event in enumerate(FILE):
    ...
    for i in range(lheEvent.product().weights().size() ):
      print "\tWeight#" + str(i) + "=", lheEvent.product().weights()[i].wgt
